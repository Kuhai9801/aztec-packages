...
MpscConsumer::MpscConsumer(std::vector<SpscShm> &&rings, int doorbell_fd,
                           size_t doorbell_len, MpscDoorbell *doorbell)
    : rings_(std::move(rings)), doorbell_fd_(doorbell_fd),
      doorbell_len_(doorbell_len), doorbell_(doorbell) {}

MpscConsumer::MpscConsumer(MpscConsumer &&other) noexcept
    : rings_(std::move(other.rings_)), doorbell_fd_(other.doorbell_fd_),
      doorbell_len_(other.doorbell_len_), doorbell_(other.doorbell_),
...
MpscConsumer MpscConsumer::create(const std::string &name, size_t num_producers,
  ...
  int doorbell_fd =
      shm_open(doorbell_name.c_str(), O_RDWR | O_CREAT | O_EXCL, 0600);
  if (doorbell_fd < 0) {
  ...
  if (ftruncate(doorbell_fd, static_cast<off_t>(doorbell_len)) != 0) {
    ...
    throw std::runtime_error(
        "MpscConsumer::create: ftruncate doorbell failed: " +
        std::string(std::strerror(e)));
  }

  auto *doorbell = static_cast<MpscDoorbell *>(
      mmap(nullptr, doorbell_len, PROT_READ | PROT_WRITE, MAP_SHARED,
           doorbell_fd, 0));
  if (doorbell == MAP_FAILED) {
    int e = errno;
    ::close(doorbell_fd);
    shm_unlink(doorbell_name.c_str());
    throw std::runtime_error("MpscConsumer::create: mmap doorbell failed: " +
                             std::string(std::strerror(e)));
  }
...
int MpscConsumer::wait_for_data(uint64_t timeout_ns) {
  ...
  // a publish that lands before we sleep returns EAGAIN. No consumer_blocked
  // flag — that conditional-wake handshake lost wakes and is gone.
  ...
  futex_wait_timeout(reinterpret_cast<volatile uint32_t *>(&doorbell_->seq),
                     seq, remaining_timeout);
...
MpscProducer::MpscProducer(SpscShm &&ring, int doorbell_fd, size_t doorbell_len,
                           MpscDoorbell *doorbell, size_t producer_id)
    : ring_(std::move(ring)), doorbell_fd_(doorbell_fd),
      doorbell_len_(doorbell_len), doorbell_(doorbell),
      producer_id_(producer_id) {}

MpscProducer::MpscProducer(MpscProducer &&other) noexcept
    : ring_(std::move(other.ring_)), doorbell_fd_(other.doorbell_fd_),
      doorbell_len_(other.doorbell_len_), doorbell_(other.doorbell_),
...
MpscProducer MpscProducer::connect(const std::string &name,
                                   size_t producer_id) {
  if (name.empty()) {
  ...
  if (doorbell == MAP_FAILED) {
    int e = errno;
    ::close(doorbell_fd);
    throw std::runtime_error("MpscProducer::connect: mmap doorbell failed: " +
                             std::string(std::strerror(e)));
  }
  ...
  try {
    ...
    return MpscProducer(std::move(ring), doorbell_fd, doorbell_len, doorbell,
                        producer_id);
  } catch (...) {
    munmap(doorbell, doorbell_len);
    ::close(doorbell_fd);
...
void MpscProducer::publish(size_t n) {
  ...
  // Ring doorbell to wake consumer. Bump seq (release) so a consumer mid-block
  // sees the value change and its futex_wait returns immediately, then wake
  // unconditionally — the conditional consumer_blocked handshake lost wakes and
  // is gone (see SpscShm::publish).
