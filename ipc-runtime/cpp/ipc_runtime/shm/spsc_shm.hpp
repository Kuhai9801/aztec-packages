/**
 * @file spsc_shm.hpp
 * @brief Single-producer/single-consumer shared-memory ring buffer (Linux,
 * x86-64 optimized)
 *
...
struct alignas(SPSC_CACHELINE) SpscCtrl {
  ...
  uint64_t wrap_head; // Head value when last message wrapped (UINT64_MAX = no
                      // wrap), synchronized by head
...
static_assert(sizeof(SpscCtrl) % SPSC_CACHELINE == 0,
 ...
 * Each claim(n)/publish(n) pair by the producer MUST be perfectly matched by a
 * corresponding peek(n)/release(n) pair by the consumer, with the EXACT same
 * sizes.
 *
 * This is because the wrapping logic is completely stateless - it decides
 * whether to wrap based solely on whether the requested size fits in the
 * remaining space before the end of the buffer. If the producer and consumer
 * use different sizes, they will make inconsistent wrap decisions and data
 * corruption will occur.
 *
 * CORRECT usage example (framed messages):
 *   Producer:                               Consumer:
 *   claim(4), publish(4)             <--->  peek(4), release(4)              //
 * length prefix claim(msg_len), publish(msg_len) <--->  peek(msg_len),
 * release(msg_len)  // message data
 */
class SpscShm {
public:
  /**
   ...
   *
   * IMPORTANT: The size passed to claim(want) must exactly match the size
   * passed to the corresponding peek(want) call by the consumer. Otherwise wrap
   ...
   *
   * IMPORTANT: The size passed to publish(n) must exactly match the size passed
   * to the corresponding release(n) call by the consumer. Otherwise wrap
   * decisions will be inconsistent.
   */
   ...
   *
   * IMPORTANT: The size passed to peek(want) must exactly match the size passed
   * to the corresponding claim(want) call by the producer. Otherwise wrap
   * decisions will be inconsistent.
   */
   ...
   *
   * IMPORTANT: The size passed to release(n) must exactly match the size passed
   * to the corresponding publish(n) call by the producer. Otherwise wrap
   * decisions will be inconsistent.
   */
   ...
   * @param prefix Prefix string for the debug output (e.g., "Client REQ" or
   * "Server RESP")
   */
...
private:
  ...
  bool previous_had_data_ = false;  // Adaptive spinning: consumer only spins if
                                    // previous call found data
  bool previous_had_space_ = false; // Adaptive spinning: producer only spins if
                                    // previous call found space
};
