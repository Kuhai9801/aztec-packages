 ...
 *
 * Coordinates multiple producers using individual SPSC rings and a shared
 * doorbell.
 */
 ...
 *
 * Manages multiple SPSC rings (one per producer) and waits on a shared
 * doorbell.
 */
class MpscConsumer {
public:
   ...
   */
  static MpscConsumer create(const std::string &name, size_t num_producers,
   ...
   * @param timeout_ns Total timeout in nanoseconds (spins 10ms, then futex
   * waits for remainder)
...
private:
  MpscConsumer(std::vector<SpscShm> &&rings, int doorbell_fd,
  ...
  bool previous_had_data_ =
      false; // Adaptive spinning: only spin if previous call found data
};
...
private:
  MpscProducer(SpscShm &&ring, int doorbell_fd, size_t doorbell_len,
