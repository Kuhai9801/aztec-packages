...
SpscShm::SpscShm(int fd, size_t map_len, SpscCtrl *ctrl, uint8_t *buf)
    : fd_(fd), map_len_(map_len), ctrl_(ctrl), buf_(buf) {}

SpscShm::SpscShm(SpscShm &&other) noexcept
    : fd_(other.fd_), map_len_(other.map_len_), ctrl_(other.ctrl_),
      buf_(other.buf_) {
  other.fd_ = -1;
  other.map_len_ = 0;
  other.ctrl_ = nullptr;
...
SpscShm SpscShm::create(const std::string &name, size_t min_capacity) {
  ...
  if (fd < 0) {
    std::string error_msg = "SpscShm::create: shm_open failed for '" + name +
                            "': " + std::strerror(errno);
  ...
  if (mem == MAP_FAILED) {
    int e = errno;
    std::string error_msg = "SpscShm::create: mmap failed for '" + name +
                            "' (size=" + std::to_string(map_len) +
                            "): " + std::strerror(e);
    if (e == ENOSPC || e == ENOMEM) {
      error_msg += " (likely /dev/shm is full - check df -h /dev/shm)";
  ...
  // Initialize atomics with release ordering to ensure capacity/mask/wrap_head
  // are visible
...
SpscShm SpscShm::connect(const std::string &name) {
  ...
  if (fd < 0) {
    throw std::runtime_error("SpscShm::connect: shm_open failed: " +
  ...
  if (fstat(fd, &st) != 0) {
    int e = errno;
    ::close(fd);
    throw std::runtime_error("SpscShm::connect: fstat failed: " +
  ...
  if (mem == MAP_FAILED) {
    int e = errno;
    ::close(fd);
    throw std::runtime_error("SpscShm::connect: mmap failed: " +
                             std::string(std::strerror(e)));
  }
...
void SpscShm::publish(size_t n) {
  ...
  // Always wake. A conditional wake gated on a consumer_blocked flag is a
  // cross-process StoreLoad (Dekker) handshake; it was observed to lose wakes
  // intermittently and deadlock the consumer on an already-published frame.
  // futex_wake with no waiter is a cheap no-op, so wake unconditionally — the
  // consumer's value-check on head still avoids the syscall in its fast path.
...
void SpscShm::release(size_t n) {
  ...
  // Always wake any producer blocked on a full ring — see publish() for why the
  // conditional producer_blocked handshake was removed.
...
bool SpscShm::wait_for_data(size_t need, uint64_t timeout_ns) {
  ...
  if (spin_duration > 0) {
    ...
    constexpr uint32_t TIME_CHECK_INTERVAL =
        256; // Check time every 256 iterations
  ...
  // syscall returns EAGAIN immediately rather than sleeping. No
  // consumer_blocked flag is needed — that conditional-wake handshake lost
  // wakes and is gone.
  uint32_t head_now =
      static_cast<uint32_t>(ctrl_->head.load(std::memory_order_acquire));
  ...
  futex_wait_timeout(reinterpret_cast<volatile uint32_t *>(&ctrl_->head),
                     head_now, remaining_timeout);
...
bool SpscShm::wait_for_space(size_t need, uint64_t timeout_ns) {
  ...
  auto check_space = [this, cap, mask, need]() -> bool {
    ...
    // std::cerr << "Checking space: head=" << head << " tail=" << tail << "
    // free=" << freeb << " need=" << need
    //           << "\n";
    if (freeb < need) {
  ...
  // sleeping. Mirrors wait_for_data — no producer_blocked flag.
  uint32_t tail_now =
      static_cast<uint32_t>(ctrl_->tail.load(std::memory_order_acquire));
  ...
  futex_wait_timeout(reinterpret_cast<volatile uint32_t *>(&ctrl_->tail),
                     tail_now, remaining_timeout);
...
void SpscShm::debug_dump(const char *prefix) const {
  ...
  std::cerr << "[" << prefix << "] head=" << head << " tail=" << tail
            << " | head_pos=" << head_pos << " tail_pos=" << tail_pos
            << " | used=" << used << " free=" << free << " cap=" << cap
            << " | wrap_head="
            << (wrap_head == UINT64_MAX ? "NONE" : std::to_string(wrap_head))
            << '\n';
}
