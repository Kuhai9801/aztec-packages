...
> Earlier revisions gated the wake on a `consumer_blocked`/`producer_blocked`
> flag (a cross-process Dekker handshake) to skip the syscall when no one was
> waiting. That handshake intermittently lost wakes and deadlocked the
> consumer on an already-published frame, so the wake is now unconditional —
> a `futex_wake` with no waiter is a cheap no-op.
