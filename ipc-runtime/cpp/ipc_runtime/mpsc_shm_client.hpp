...
public:
  MpscShmClient(std::string base_name, size_t client_id)
      : base_name_(std::move(base_name)), client_id_(client_id) {}
    ...
    for (size_t attempt = 0; attempt < max_attempts; ++attempt) {
      try {
        ...
        std::string resp_name =
            base_name_ + "_resp_" + std::to_string(client_id_);
  ...
  bool send(const void *data, size_t len, uint64_t timeout_ns) override {
    ...
    void *buf =
        producer_->claim(total_size, normalize_call_timeout(timeout_ns));
    if (buf == nullptr) {
      return false;
    }
  ...
  std::span<const uint8_t> receive(uint64_t timeout_ns) override {
    ...
    return ring_receive_msg(response_ring_.value(),
                            normalize_call_timeout(timeout_ns));
  }
